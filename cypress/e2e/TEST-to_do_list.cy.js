...
describe('TEST to do list', () => {
  ...
  it('Generar una nueva tarea', () => {
    ...
    cy.get('#tarea')
      .type('Mi tarea cargada por Cypress en TEST')
      .should('have.value', 'Mi tarea cargada por Cypress en TEST');
    cy.get('#enviar').click();
  });
  ...
  it('Cargar una segunda tarea', () => {
    cy.get('#nav-tareas').click();
    cy.get('#tarea')
      .type('Mi 2da tarea cargada por Cypress en TEST')
      .should('have.value', 'Mi 2da tarea cargada por Cypress en TEST');
    cy.get('#enviar').click();
  });
  ...
  it('Validar minimo de caracteres para cargar una tarea', () => {
    cy.get('#nav-tareas').click();
    cy.get('#tarea').type('Tarea').should('have.value', 'Tarea');
    cy.get('#enviar').click();
  ...
  it('Modifico tarea existente - 2da tarea cargada', () => {
    ...
    cy.get('#tarea')
      ...
      .should('have.value', '')
      .type('Mi 2da tarea por Cypress MODIFICADA')
      .should('have.value', 'Mi 2da tarea por Cypress MODIFICADA');
    cy.get('#enviar').click();
  });
